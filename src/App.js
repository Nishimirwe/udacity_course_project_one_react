...
class BooksApp extends React.Component {
  ...
  // and it will be passed in searching component.
   ...
   setCurrentBook=(data)=>
   {
    this.setState({currentBook: data.id});
   }
  ...
  {
    let conf=window.confirm("Do you want to move to "+this.whereToMove()+" category ? Y/N");
      if(conf === true)
    {
      if(this.state.move==="cr")
      {
  ...
  setMove=(e)=>
  {
    try{
      ...
      formObj.leaveCode[0].selected = true;
    }catch(e){
        if(e){
        this.setState({move: "no"})
        }
      ...
      <div className="app">
        <Route path="/search">
           <Search moveItem={(data,from)=>{this.shiftCategories(data,from)}}
            ...
            setCurrentBook={(d)=>this.setCurrentBook(d)}
           ...
           <div className="list-books">
            ...
            <div className="list-books-content">
              <div>
                ...
                currentBook={this.state.currentBook}
                setCurrentBook={(d)=>this.setCurrentBook(d)} 
                setCurrentCtgr={()=>this.setState({currentCtgr: "cr"})} ctgr={this.state.currentCtgr} 
                moveState={this.state.move} changeMove={(e)=>this.setMove(e)} moveItem={(data)=>{
                  this.shiftCategories(data,"cr")}} />
                  
                <WTRead ReadsData={this.state.WantToRead} 
                currentBook={this.state.currentBook}
                setCurrentBook={(d)=>this.setCurrentBook(d)} 
                setCurrentCtgr={()=>this.setState({currentCtgr: "wr"})} ctgr={this.state.currentCtgr} 
                moveState={this.state.move} changeMove={(e)=>this.setMove(e)}  moveItem={(data)=>{
                ...
                currentBook={this.state.currentBook}
                setCurrentBook={(d)=>this.setCurrentBook(d)} 
                setCurrentCtgr={()=>this.setState({currentCtgr: "r"})} ctgr={this.state.currentCtgr} 
                moveState={this.state.move} changeMove={(e)=>this.setMove(e)}  moveItem={(data)=>{
